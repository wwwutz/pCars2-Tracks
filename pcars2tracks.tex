\documentclass[oneside, a4paper, 12pt]{book}
% \usepackage[ngerman]{babel}
\usepackage{graphicx}
\usepackage{calc}
\usepackage{amsmath}
\usepackage{fancyvrb}
\usepackage[utf8]{inputenc}
\pagestyle{empty}
\usepackage[absolute]{textpos}
%\usepackage[absolute,showboxes]{textpos}

\usepackage[]{eso-pic}[2002/11/16]
\usepackage{fancybox} % fuer Oval...
\renewcommand\familydefault{\sfdefault}
\pdfminorversion=4
%
% TOC stuff
%
\usepackage[colorlinks=true,linkcolor=blue]{hyperref}
\usepackage{paralist}
\usepackage{longtable}

%\usepackage{showframe}

\setlength{\textheight}{24cm}
\addtolength{\headsep}{-1in}
\addtolength{\textwidth}{2in}
\addtolength{\oddsidemargin}{-1in}
\addtolength{\evensidemargin}{-1in}
\setlength{\parindent}{0pt}

\newlength{\HT}
% ICON{name}{reftext for size}
\newcommand*{\ICON}[2]{\settoheight{\HT}{\hbox{#2}}\mbox{\includegraphics[height=\HT,keepaspectratio]{icons/#1.pdf}}}

\definecolor{grey1}{gray}{0.75}
\definecolor{grey0}{gray}{0.55}

\makeatletter
\newcommand*{\textatop}[2]{$\hbox{#1}\atop\hbox{#2}\m@th$}
\makeatother

\begin{document}
\fontfamily{phv}\selectfont
\textblockorigin{0cm}{0cm}

\newlength{\Logo}
\setlength{\Logo}{210mm-90mm}
\begin{textblock*}{\Logo}(45mm,40mm)%
\includegraphics[width=\Logo]{pcars2-main.png}
\end{textblock*}

%\begin{textblock*}{\Logo}(30mm,125mm)%
%\begin{center}\includegraphics[height=15mm,keepaspectratio]{icons/project-cars.pdf}\end{center}
%\end{textblock*}

\begin{textblock*}{30mm}(60mm,125mm)%
\resizebox{90mm}{25mm}{Tracks}
\end{textblock*}

\begin{textblock*}{90mm}(60mm,210mm)%
\begin{center}\textatop{\Huge{v1.0 / 9. Oct 2017}}{\small{based on $\stackrel{\text{\sffamily PROJECT}}{\text{\textbf{CARS///}}}$\LARGE{\sffamily 2}  \small\texttt{V1.1.3.2.0882.AVX}}}\end{center}
\end{textblock*}

\begin{textblock*}{90mm}(60mm,230mm)%
\begin{center}\ICON{fa-github}{X} \tt{https://github.com/wwwutz/pCars2-Tracks}\end{center}
\end{textblock*}

\include{logos-fp}
\null\newpage
\include{all-toc}
\include{all-tracks}

\end{document}
